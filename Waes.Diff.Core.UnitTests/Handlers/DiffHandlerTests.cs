...
namespace Waes.Diff.Core.UnitTests.Handlers
{
    public class DiffHandlerTests
    {
        ...
        {
            sut.BinaryDataStorage.Get("left" + id).Returns(data1);
            sut.BinaryDataStorage.Get("right" + id).Returns(data2);
        ...
        {
            sut.BinaryDataStorage.Get("left" + id).Returns((byte[])null);
            sut.BinaryDataStorage.Get("right" + id).Returns(data2);
            
            var exception = await Assert.ThrowsAsync<BinaryDataNotFoundException>(() => sut.Diff(id));
            exception.Id.Should().Be("left" + id);
        }
        ...
        {
            sut.BinaryDataStorage.Get("left" + id).Returns(data1);
            sut.BinaryDataStorage.Get("right" + id).Returns((byte[])null);

            var exception = await Assert.ThrowsAsync<BinaryDataNotFoundException>(() => sut.Diff(id));
            exception.Id.Should().Be("right" + id);
        }
    }
}